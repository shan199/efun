...
class XingeApp
{
     ...
     */
    public static function PushTagAndroid($accessId, $secretKey, $title, $content, $tag, $custom, $activity = '')
    {
        $push = new XingeApp($accessId, $secretKey);
        $mess = new Message();
        $mess->setTitle($title);
        ...
        $action = new ClickAction();
        $action->setActionType(ClickAction::TYPE_ACTIVITY);
        $action->setActivity($activity); //打开页面
     ...
     * 查询token的标签
     */
    protected function QueryTokenTags($deviceToken)
    {
        $ret = array('ret_code' => -1);
