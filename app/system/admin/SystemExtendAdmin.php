        ...
        if (method_exists($this, '_indexParam')) {
            ...
            if (!empty($indexParam)) {
                foreach ($indexParam as $key => $val) {
                    ...
                    $value = Str::htmlClear($value);


                    if ($key == 'keyword') {

                        //搜索多个条件
                        if (is_array($indexParam['keyword'])) {
                            $str = '';
                            foreach ($indexParam['keyword'] as $objitem) {
                                $str .= "{$objitem} like '%{$value}%'" . " OR ";
                            }
                            if (!empty($value)) {
                                $whereMaps[] = '( ' . substr($str, 0, -4) . ' )';
                            }
                        } else {
                            if (!empty($value)) {
                                $whereMaps[] = "{$val} like '%{$value}%'";
                            }
                        }
